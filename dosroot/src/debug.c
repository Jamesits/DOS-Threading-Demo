...
void far print_tcb() {
    ...
    lprintf(INFO, "\ttcb_count: %d, schedule_reent: %d, in_kernel: %d\n", tcb_count, schedule_reent, in_kernel);
    lprintf(INFO, "\tPID\tName\t\tTCBptr\t\tStack\t\tSS:SP\t\tState\n");
    for (i = 0; i < tcb_count; ++i) {
        ...
        lprintf(INFO, "\t%d\t%s\t\t0x%Fp\t0x%Fp\t0x%Np:0x%Np\t%d %s\n", tcb[i].uid, tcb[i].name, tcb + i, tcb[i].stack, tcb[i].ss, tcb[i].sp, current_thread_state, tcb_status_text[current_thread_state]);
    }
    //end_transaction();
#endif
}
